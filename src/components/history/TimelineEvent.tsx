...
export default function TimelineEvent({ date, title, description, significance, imageUrl, isLeft, delay = 0 }: TimelineEventProps) {
  return (
    <div className="relative mb-12 z-10 w-full flex flex-col md:flex-row items-center md:items-start">
      ...
      <div className="hidden md:block absolute left-1/2 w-4 h-4 rounded-full bg-[#FF6B00] border-4 border-[#FFF8F0] transform -translate-x-1/2 z-20 mt-8"></div>

      {/* Mobile Dot */}
      <div className="md:hidden absolute left-1.5 w-4 h-4 rounded-full bg-[#FF6B00] border-4 border-[#FFF8F0] transform -translate-x-1/2 z-20 mt-8"></div>
      ...
      <div className={`w-full md:w-1/2 ${isLeft ? 'md:pr-12' : 'md:pl-12 md:ml-auto'} pl-8 md:pl-${isLeft ? '0' : '12'}`}>
        <motion.div 
          ...
          className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-lg shadow-black/5 max-w-[380px] w-full"
        ...
        >
          ...
          <div className="p-5">
            <h4 className="font-[family-name:var(--font-baloo-2)] text-lg font-bold text-[#1A1A2E] mb-2 leading-tight">
              {title}
            </h4>
            <p className="font-[family-name:var(--font-inter)] text-sm leading-relaxed text-[#1A1A2E]/60 mb-3">
              {description}
            </p>
